...
/// <summary>
/// يضمن وجود جداول طلب الصرف المطلوبة للتشغيل.
/// الأوامر آمنة لإعادة التنفيذ ولا تحذف أو تعدل بيانات موجودة.
/// </summary>
public sealed class PaymentRequestSchemaInitializer
{
    ...
    public PaymentRequestSchemaInitializer(
        AppDbContext db,
        ILogger<PaymentRequestSchemaInitializer> logger)
    {
        _db = db;
        _logger = logger;
    }
...
CREATE TABLE IF NOT EXISTS payment_requests (
    ...
    Header_Reference_No VARCHAR(100) NULL,
    Description VARCHAR(500) NULL,
    Approved_Local_Total DECIMAL(19,4) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS payment_request_lines (
    ...
    Foreign_Amount DECIMAL(19,4) NOT NULL DEFAULT 0,
    Local_Amount DECIMAL(19,4) NOT NULL,
    Reference_No VARCHAR(100) NULL,
    Description VARCHAR(500) NULL,
    PRIMARY KEY (Payment_Request_Line_ID),
    UNIQUE KEY UQ_payment_request_lines_no (Payment_Request_ID, Line_No),
...
CREATE TABLE IF NOT EXISTS payment_request_attachments (
    ...
    Created_By VARCHAR(50) NOT NULL,
    Created_At DATETIME NOT NULL,
    PRIMARY KEY (Payment_Request_Attachment_ID),
    KEY IX_payment_request_attachments_scope (Payment_Request_ID, Is_Active)
) ENGINE=InnoDB;
""";

        await _db.Database.ExecuteSqlRawAsync(sql, cancellationToken);
        _logger.LogInformation("تم التحقق من جداول طلب الصرف بنجاح.");
    }
}

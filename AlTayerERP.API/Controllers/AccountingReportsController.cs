...
public sealed class AccountingReportsController : ControllerBase
{
    ...
    public async Task<IActionResult> TrialBalance([FromQuery]DateTime? toDate=null, [FromQuery]int? currencyId=null, [FromQuery]string? costCenterId=null)
    {
        ...
        var rows=await (from d in _db.Financial_Voucher_Details.AsNoTracking()
                        ...
                        where h.Is_Active&&h.Is_Posted&&h.Branch_ID==s.Branch_ID.ToString()&&h.Fiscal_Year_ID==s.Year_ID&&h.Voucher_Date<=date&&a.Company_ID==s.Company_ID&&(currencyId==null||d.Currency_ID==currencyId.Value)&&(string.IsNullOrWhiteSpace(costCenterId)||d.Cost_Center_ID==costCenterId.Trim())
    ...
    public async Task<IActionResult> GeneralLedger([FromQuery]string accountId,[FromQuery]DateTime? fromDate=null,[FromQuery]DateTime? toDate=null,[FromQuery]int? currencyId=null,[FromQuery]string? costCenterId=null)
    {
        ...
        var opening=await ((from d in _db.Financial_Voucher_Details.AsNoTracking() join h in _db.Financial_Voucher_Headers.AsNoTracking() on d.Voucher_ID equals h.Voucher_ID
                           where h.Is_Active&&h.Is_Posted&&h.Branch_ID==s.Branch_ID.ToString()&&h.Fiscal_Year_ID==s.Year_ID&&d.Account_ID==accountId.Trim()&&h.Voucher_Date<startDate&&(currencyId==null||d.Currency_ID==currencyId.Value)&&(string.IsNullOrWhiteSpace(costCenterId)||d.Cost_Center_ID==costCenterId.Trim())
        ...
        var entries=await ((from d in _db.Financial_Voucher_Details.AsNoTracking() join h in _db.Financial_Voucher_Headers.AsNoTracking() on d.Voucher_ID equals h.Voucher_ID
                           where h.Is_Active&&h.Is_Posted&&h.Branch_ID==s.Branch_ID.ToString()&&h.Fiscal_Year_ID==s.Year_ID&&d.Account_ID==accountId.Trim()&&h.Voucher_Date>=startDate&&h.Voucher_Date<=to&&(currencyId==null||d.Currency_ID==currencyId.Value)&&(string.IsNullOrWhiteSpace(costCenterId)||d.Cost_Center_ID==costCenterId.Trim())

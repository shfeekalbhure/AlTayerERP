...
public sealed class FrmPaymentRequest : BaseForm
{
    ...
    private readonly TextBox _paymentMethod=new(){Width=120,PlaceholderText="معرف طريقة السداد"};
    ...
    private void Build()
    {
        ...
        head.Controls.AddRange(new Control[]{L("رقم الطلب"),_number,L("المستفيد"),_beneficiary,L("الطرف"),_party,L("طريقة السداد"),_paymentMethod,L("المرجع"),_reference,L("البيان"),_description,_reason,_cash});
    ...
    private void NewRequest(){_id=0;_rowVersion=null;_status="DRAFT";_paymentVoucherId=null;ApplyWorkflowState();_number.Clear();_beneficiary.Clear();_party.Clear();_paymentMethod.Clear();_reference.Clear();_description.Clear();_reason.Clear();_cash.Clear();_lines.Rows.Clear();_lines.Rows.Add();_lines.Rows.Add();}
    private static decimal D(DataGridViewRow r,string name)=>decimal.TryParse(r.Cells[name].Value?.ToString(),out var x)?x:0;
    private static string SafeApiMessage(string content,string fallback){try{using var document=JsonDocument.Parse(content);if(document.RootElement.TryGetProperty("message",out var message)&&message.ValueKind==JsonValueKind.String&&!string.IsNullOrWhiteSpace(message.GetString()))return message.GetString()!;}catch(JsonException){}return fallback;}
    private object Payload()=>new{request_Date=DateTime.Today,RowVersion = _rowVersion,beneficiary_Name=_beneficiary.Text.Trim(),party_ID=string.IsNullOrWhiteSpace(_party.Text)?null:_party.Text.Trim(),payment_Method_ID=int.TryParse(_paymentMethod.Text.Trim(),out var paymentMethodId)?paymentMethodId:(int?)null,header_Reference_No=string.IsNullOrWhiteSpace(_reference.Text)?null:_reference.Text.Trim(),description=_description.Text.Trim(),lines=_lines.Rows.Cast<DataGridViewRow>().Where(r=>!r.IsNewRow&&!string.IsNullOrWhiteSpace(r.Cells["Account"].Value?.ToString())).Select(r=>new{account_ID=r.Cells["Account"].Value!.ToString(),cost_Center_ID=r.Cells["CostCenter"].Value?.ToString(),currency_ID=int.TryParse(r.Cells["Currency"].Value?.ToString(),out var c)?c:0,exchange_Rate=D(r,"Rate"),foreign_Amount=D(r,"Foreign"),local_Amount=D(r,"Local"),reference_No=r.Cells["Reference"].Value?.ToString(),description=r.Cells["Description"].Value?.ToString()}).ToList()};
    private static Guid? RowVersion(JsonElement x)=>x.TryGetProperty("rowVersion",out var value)&&value.ValueKind==JsonValueKind.String&&Guid.TryParse(value.GetString(),out var parsed)?parsed:null;
    private async Task SaveAsync(){if(string.IsNullOrWhiteSpace(_beneficiary.Text)){MessageBox.Show("المستفيد مطلوب.");return;}if(!int.TryParse(_paymentMethod.Text.Trim(),out var paymentMethodId)||paymentMethodId<=0){MessageBox.Show("معرف طريقة السداد الصحيح مطلوب.");_paymentMethod.Focus();return;}var response=_id==0?await ApiService.Client.PostAsJsonAsync("payment-requests",Payload()):await ApiService.Client.PutAsJsonAsync($"payment-requests/{_id}",Payload());var text=await response.Content.ReadAsStringAsync();if(!response.IsSuccessStatusCode){MessageBox.Show(SafeApiMessage(text,"تعذر حفظ طلب الصرف. حاول مرة أخرى أو راجع الإدارة."),"طلب الصرف");return;}using var d=JsonDocument.Parse(text);_id=d.RootElement.GetProperty("payment_Request_ID").GetInt64();_rowVersion=RowVersion(d.RootElement);if(d.RootElement.TryGetProperty("request_No",out var no))_number.Text=no.GetString()??"";if(d.RootElement.TryGetProperty("status",out var status))_status=status.GetString()??"DRAFT";_paymentVoucherId=d.RootElement.TryGetProperty("payment_Voucher_ID",out var paymentVoucher)&&paymentVoucher.ValueKind!=JsonValueKind.Null?paymentVoucher.GetInt64():null;ApplyWorkflowState();MessageBox.Show("تم حفظ طلب الصرف كمسودة.");}
    private async Task SearchAsync(){if(string.IsNullOrWhiteSpace(_number.Text)){_number.Focus();return;}var raw=await ApiService.Client.GetStringAsync($"payment-requests?requestNo={Uri.EscapeDataString(_number.Text.Trim())}");using var list=JsonDocument.Parse(raw);var first=list.RootElement.EnumerateArray().FirstOrDefault();if(first.ValueKind==JsonValueKind.Undefined){MessageBox.Show("لم يتم العثور على الطلب.");return;}_id=first.GetProperty("payment_Request_ID").GetInt64();var r=await ApiService.Client.GetAsync($"payment-requests/{_id}");if(!r.IsSuccessStatusCode){MessageBox.Show(SafeApiMessage(await r.Content.ReadAsStringAsync(),"تعذر تحميل طلب الصرف."));return;}using var doc=JsonDocument.Parse(await r.Content.ReadAsStringAsync());var x=doc.RootElement;_rowVersion=RowVersion(x);_number.Text=x.GetProperty("request_No").GetString()??"";_beneficiary.Text=x.GetProperty("beneficiary_Name").GetString()??"";_party.Text=x.TryGetProperty("party_ID",out var p)?p.GetString()??"":"";_paymentMethod.Text=x.TryGetProperty("payment_Method_ID",out var pm)&&pm.ValueKind!=JsonValueKind.Null?pm.GetInt32().ToString():"";_reference.Text=x.TryGetProperty("header_Reference_No",out var rf)?rf.GetString()??"":"";_description.Text=x.TryGetProperty("description",out var de)?de.GetString()??"":"";_lines.Rows.Clear();foreach(var line in x.GetProperty("details").EnumerateArray())_lines.Rows.Add(line.GetProperty("account_ID").GetString(),line.TryGetProperty("cost_Center_ID",out var cc)?cc.GetString():null,line.GetProperty("currency_ID").GetInt32(),line.GetProperty("exchange_Rate").GetDecimal(),line.GetProperty("foreign_Amount").GetDecimal(),line.GetProperty("local_Amount").GetDecimal(),line.TryGetProperty("reference_No",out var rr)?rr.GetString():null,line.TryGetProperty("description",out var dd)?dd.GetString():null);_status=x.TryGetProperty("status",out var status)?status.GetString()??"DRAFT":"DRAFT";_paymentVoucherId=x.TryGetProperty("payment_Voucher_ID",out var paymentVoucher)&&paymentVoucher.ValueKind!=JsonValueKind.Null?paymentVoucher.GetInt64():null;ApplyWorkflowState();}
    ...
    private async Task ActionAsync(string action,bool reason)
    {
        ...
        if(!r.IsSuccessStatusCode){MessageBox.Show(SafeApiMessage(responseText,"تعذر تنفيذ الإجراء على طلب الصرف."));return;}
    ...
    private async Task CreateVoucherAsync()
    {
        ...
        var responseText=await r.Content.ReadAsStringAsync();
        if(!r.IsSuccessStatusCode){MessageBox.Show(SafeApiMessage(responseText,"تعذر إنشاء سند الصرف المرتبط."));return;}
        using var document=JsonDocument.Parse(responseText);

...
public sealed class FrmPaymentRequest : BaseForm
{
    ...
    private void Build()
    {
        ...
        bar.Controls.AddRange(new Control[]{B("جديد F2",(_,_)=>NewRequest()),B("حفظ Ctrl+S",async(_,_)=>await SaveAsync()),B("بحث",async(_,_)=>await SearchAsync()),B("إرسال للمراجعة",async(_,_)=>await ActionAsync("submit",false)),B("مراجعة",async(_,_)=>await ActionAsync("review",true)),B("اعتماد",async(_,_)=>await ActionAsync("approve",true)),B("رفض",async(_,_)=>await ActionAsync("reject",true)),B("إرجاع",async(_,_)=>await ActionAsync("return",true)),B("مرفقات",async(_,_)=>await UploadAttachmentAsync()),B("إنشاء سند الصرف",async(_,_)=>await CreateVoucherAsync())});
    ...
    private void NewRequest(){_id=0;_number.Clear();_beneficiary.Clear();_party.Clear();_reference.Clear();_description.Clear();_reason.Clear();_cash.Clear();_lines.Rows.Clear();_lines.Rows.Add();_lines.Rows.Add();}

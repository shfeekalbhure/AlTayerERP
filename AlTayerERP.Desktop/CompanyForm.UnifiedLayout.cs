...
public partial class CompanyForm
{
    ...
    private void ApplyApprovedCompanyLayout()
    {
        ...
        var search = new TextBox { Dock = DockStyle.Fill, PlaceholderText = "ابحث بكود الشركة أو الاسم أو الهاتف أو الرقم الضريبي…", Margin = new Padding(0, 5, 0, 5) };
        ...
        var gridCard = Card("قائمة الشركات", dgvCompanies);
        shell.Controls.Add(gridCard, 0, 4);

        shell.Controls.Add(new Label { Text = "ترتبط كل شركة بمجموعة تجارية واحدة، ولا يتم تغيير الحالة إلا عبر الإيقاف أو إعادة التفعيل مع التدقيق.", Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleRight, ForeColor = Color.FromArgb(55, 85, 130) }, 0, 5);
        Controls.Add(shell);
        ResumeLayout(true);
    }
    ...
    private Control BuildCompanyIdentityCard()
    {
        ...
        var card = Card("الهوية القانونية", table);
        return card;
    }
    ...
    private Control BuildCompanyContactCard()
    {
        ...
        var logoButtons = new FlowLayoutPanel { Dock = DockStyle.Bottom, Height = 38, FlowDirection = FlowDirection.RightToLeft };
    ...
    private static Control Input(Control c) { c.Dock = DockStyle.Fill; c.Margin = new Padding(4, 7, 4, 7); return c; }
    ...
    {
        var p = new Panel { Dock = DockStyle.Fill, BackColor = Color.White, BorderStyle = BorderStyle.FixedSingle, Padding = new Padding(8) };
        body.Dock = DockStyle.Fill;
        p.Controls.Add(body);
        p.Controls.Add(new Label { Text = title, Dock = DockStyle.Top, Height = 30, TextAlign = ContentAlignment.MiddleRight, Font = new Font("Segoe UI", 10.5F, FontStyle.Bold), ForeColor = Color.FromArgb(8, 55, 112) });
        return p;
    }
}

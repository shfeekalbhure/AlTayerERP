...
public class FrmGeographicReference : BaseForm
{
    ...
    private void Build()
    {
        ...
        shell.Controls.Add(Title(Text), 0, 0);
        shell.Controls.Add(Toolbar(), 0, 1);
        shell.Controls.Add(Editor(), 0, 2);
        _search.Dock = DockStyle.Fill; _search.Margin = new Padding(0, 5, 0, 5); _search.TextChanged += (_, _) => Filter();
        shell.Controls.Add(_search, 0, 3);
        shell.Controls.Add(GridCard(), 0, 4);
        shell.Controls.Add(Footer(), 0, 5);
    ...
    private Control Toolbar()
    {
        ...
        {
            Button("جديد  Ctrl+N", (_, _) => ClearForm()), _save,
            Button("تعديل", async (_, _) => await SaveAsync()), _deactivate, _reactivate,
            Button("تحديث  F5", async (_, _) => await LoadRowsAsync()),
    ...
    private Control Editor()
    {
        ...
        Field(table, $"كود {Singular} *", _code, 0, 0);
        Field(table, $"اسم {Singular} بالعربية *", _nameAr, 1, 0);
        Field(table, $"اسم {Singular} بالإنجليزية", _nameEn, 2, 0);
        Field(table, "ترتيب الظهور", _sort, 3, 0);
        ...
        if (_type == GeographicReferenceType.Country)
        {
            ...
            Field(table, "اسم الجنسية بالعربية", _nationalityAr, 0, 2, 2);
        ...
        else
        {
            ...
            {
                Field(table, "المحافظة *", _governorate, 1, 1);
                Field(table, "الرمز البريدي", _postalCode, 2, 1);
            }
            Field(table, "الحالة", _active, 3, 2);
        }
    ...
    private Control Footer()
    {
        ...
        p.Controls.Add(_count);
        p.Controls.Add(new Label { Text = "الترابط المعتمد: الدولة ← المحافظة ← المدينة. الإيقاف وإعادة التفعيل يتطلبان سبباً ويسجلان في التدقيق.", Dock = DockStyle.Right, Width = 760, TextAlign = ContentAlignment.MiddleRight, ForeColor = Color.FromArgb(75, 85, 99) });
        return p;
    }
    ...
    private async Task LoadCountriesAsync(bool preserve)
    {
        ...
        _country.DataSource = data; _country.DisplayMember = nameof(CountryLookup.Country_Name_AR); _country.ValueMember = nameof(CountryLookup.Country_ID);
    ...
    private async Task LoadGovernoratesAsync(bool preserve, int requestedId = 0)
    {
        ...
        var data = countryId > 0
            ...
            : new();
        _governorate.DataSource = data; _governorate.DisplayMember = nameof(GovernorateLookup.Governorate_Name_AR); _governorate.ValueMember = nameof(GovernorateLookup.Governorate_ID);
    ...
    {
        string G(string key) => row.TryGetValue(key, out var value) ? value.ToString() : string.Empty;
        var status = IsTrue(G("Is_Active")) ? "نشط" : "موقوف";
        return _type switch
        {
            GeographicReferenceType.Country => new { ID = G("Country_ID"), الكود = G("Country_Code"), الاسم_العربي = G("Country_Name_AR"), الاسم_الإنجليزي = G("Country_Name_EN"), الجنسية = G("Nationality_Name_AR"), المفتاح = G("Phone_Code"), العملة = G("Currency_Code"), الحالة = status },
    ...
    private async Task SelectCurrentAsync()
    {
        ...
        string G(string key) => row.TryGetValue(key, out var value) ? value.ToString() : string.Empty;
        _code.Text = G(_type == GeographicReferenceType.Country ? "Country_Code" : _type == GeographicReferenceType.Governorate ? "Governorate_Code" : "City_Code");
        _nameAr.Text = G(_type == GeographicReferenceType.Country ? "Country_Name_AR" : _type == GeographicReferenceType.Governorate ? "Governorate_Name_AR" : "City_Name_AR");
        _nameEn.Text = G(_type == GeographicReferenceType.Country ? "Country_Name_EN" : _type == GeographicReferenceType.Governorate ? "Governorate_Name_EN" : "City_Name_EN");
        _sort.Value = decimal.TryParse(G("Sort_Order"), out var sort) ? sort : 0;
        _active.Checked = IsTrue(G("Is_Active")); _notes.Text = G("Notes");
        if (_type == GeographicReferenceType.Country)
        {
            _iso2.Text = G("ISO2"); _iso3.Text = G("ISO3"); _phoneCode.Text = G("Phone_Code");
            _currencyCode.Text = G("Currency_Code"); _nationalityAr.Text = G("Nationality_Name_AR");
        }
        else
        {
            _loadingLookups = true;
            try
            {
                _country.SelectedValue = Convert.ToInt32(G("Country_ID"));
                if (_type == GeographicReferenceType.City)
                {
                    await LoadGovernoratesAsync(false, Convert.ToInt32(G("Governorate_ID")));
                    _postalCode.Text = G("Postal_Code");
                }
            }
            finally { _loadingLookups = false; }
        }
    ...
    private async Task SaveAsync()
    {
        ...
        if (validation is not null) { MessageBox.Show(validation, Text, MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
        object dto = _type switch
        {
            GeographicReferenceType.Country => new
            {
                Country_ID = _selectedId, Country_Code = _code.Text.Trim().ToUpperInvariant(), Country_Name_AR = _nameAr.Text.Trim(), Country_Name_EN = Text(_nameEn.Text),
                ISO2 = Text(_iso2.Text)?.ToUpperInvariant(), ISO3 = Text(_iso3.Text)?.ToUpperInvariant(), Phone_Code = Text(_phoneCode.Text),
                Currency_Code = Text(_currencyCode.Text)?.ToUpperInvariant(), Nationality_Name_AR = Text(_nationalityAr.Text), Sort_Order = (int)_sort.Value, Notes = Text(_notes.Text)
            },
            GeographicReferenceType.Governorate => new
            {
                ...
                Governorate_Name_AR = _nameAr.Text.Trim(), Governorate_Name_EN = Text(_nameEn.Text), Sort_Order = (int)_sort.Value, Notes = Text(_notes.Text)
            },
            _ => new
            {
                ...
                City_Code = _code.Text.Trim().ToUpperInvariant(), City_Name_AR = _nameAr.Text.Trim(), City_Name_EN = Text(_nameEn.Text),
                Postal_Code = Text(_postalCode.Text), Sort_Order = (int)_sort.Value, Notes = Text(_notes.Text)
            }
        };
        var response = await ApiService.Client.PostAsJsonAsync($"GeographicReferences/{Resource}", dto);
        if (!response.IsSuccessStatusCode)
        {
            MessageBox.Show(await response.Content.ReadAsStringAsync(), "تعذر الحفظ", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
        }
    ...
    private async Task ChangeStatusAsync(bool reactivate)
    {
        ...
        {
            using var request = new HttpRequestMessage(HttpMethod.Delete, $"GeographicReferences/{Resource}/{_selectedId}") { Content = JsonContent.Create(new { Reason = reason }) };
    ...
    {
        var b = new Button { Text = text, Width = 132, Height = 34, Margin = new Padding(4), FlatStyle = FlatStyle.Flat, BackColor = primary ? Color.FromArgb(14, 93, 216) : Color.White, ForeColor = primary ? Color.White : danger ? Color.Firebrick : Color.FromArgb(8, 55, 112) };
        b.FlatAppearance.BorderColor = Color.FromArgb(205, 217, 232); b.Click += handler; return b;
    }
